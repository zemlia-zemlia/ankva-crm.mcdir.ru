    ...
    private function addWatermark($image_path){
        ...
        if ($image_path) {
           try {
                ...
                $watermark = Image::getImagine()->open(Yii::$app->context->watermark);
                ...
                $sizeWat = getimagesize(Yii::$app->context->watermark);        // Определяем размер водяного знака
                ...
                if (!file_exists($path . $file_name)) {
                    try {
                        if (copy($image_file, $path . $file_name)) {
                            $this->addWatermark($path . $file_name);
